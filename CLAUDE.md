...
- Calendar states (computed in `_calendar_states()` in `services/theme_rules.py`) are non-exclusive: `empty` (zero events today), `done` (had events but all `end <= now`), `active` (event with `start <= now < end`), `upcoming_soon` (event starts within `UPCOMING_SOON_MINUTES` = 30 min), `busy` (≥ `BUSY_DAY_THRESHOLD` = 5 events), `birthday_today` (any `Birthday` whose month/day matches today). A busy day with an in-progress event reports both `busy` and `active`; a rule listing either matches.
