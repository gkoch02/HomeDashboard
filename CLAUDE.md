...
- `_fetch_from_ical()` uses `requests.get(..., timeout=30)` and the `icalendar` library; HTTP or parse errors are logged per feed and then raise `CalendarFetchError` once every URL has been tried, so one run's log names all of them
