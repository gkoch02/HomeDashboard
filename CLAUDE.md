...
- `halftone_agenda` is the split-plate sibling of `halftone`, rendered by `halftone_agenda_panel.py` via the full-canvas `halftone_agenda` region. The left 372 px carry the shared `skyart.draw_weather_scene` illustration (at `SCENE_SCALE = 0.72`) over a typeset weather band; a 6-px vertical Bayer rule divides the plate; the right 422 px are the agenda. Notable points:
  ...
  - **The elapsed-row screen cut rises as the type shrinks** (`past_screen`): 96 (≈38% of the coverage) for the display tiers, 128 and 160 for the denser ones. One fixed cut leaves the 17-px type of a packed day as a row of dots. Note `skyart.screened_paste`'s cut runs the opposite way to how it reads — a **higher** threshold keeps **more** ink.
  - **`+N more` is plain ink, not screened.** A past row has its neighbours to read against; this line sits alone under the last row, and it is the one mark saying the day continues past what is shown.
