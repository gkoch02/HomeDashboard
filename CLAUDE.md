...
- `halftone_agenda` is the split-plate sibling of `halftone`, rendered by `halftone_agenda_panel.py` via the full-canvas `halftone_agenda` region. The left 372 px carry the shared `skyart.draw_weather_scene` illustration (at `SCENE_SCALE = 0.72`) over a typeset weather band; a 6-px vertical Bayer rule divides the plate; the right 422 px are the agenda. Notable points:
  ...
  - **Rows show a time range**, via `compact_range` (elides a shared meridiem: `12:30p`+`2p` → `12:30–2p`, which is what makes it fit) and `event_times` (all-day and past-midnight events fall back to the start alone). The two densest tiers carry time columns wider than their type needs so the worst case (`11:30a–1:15p`) fits on one line — their rows are too short to wrap, and unlike the roomy tiers they have title width to spare. Roomier tiers wrap instead, because widening there would cost a truncation on titles that fit today. A test asserts every tier can either fit or wrap the worst case, so no tier silently drops the end time.
