...
- ICS recurrence (RRULE / RDATE / EXDATE / RECURRENCE-ID) is expanded via the `recurring-ical-events` package (core dependency) in `_expand_components()` — one VEVENT per occurrence inside the fetch window, matching the CalDAV backend's `server_expand=True`. If the package is missing (stale deployment), the fetcher logs a warning and falls back to the raw walk, where recurring series appear only in their first week
