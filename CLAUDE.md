...
└── render/
    ...
    ├── themes/                # themes (35 — 34 concrete + `default` pseudo): standard week-view
    ...
    │                          #   almanac, scorecard, tides, halftone, trends, weatherglass);
...
| `Righteous-Regular.ttf` (OFL) | `righteous` | `light_cycle` — hero day-of-month numeral |
...
- `theme_rules` is web-editable via a YAML textarea (Advanced mode): the JS sends the textarea text under the `theme_rules` patch key; `config_editor._normalise_patch()` parses it server-side (parse failures and non-list shapes are structured errors that block the save), `_apply_to_raw()` stores the parsed list (empty list removes the key), and `get_config_for_web()` serves it back as `theme_rules_yaml` for lossless round-tripping.
