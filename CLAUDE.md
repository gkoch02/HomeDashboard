...
- **A theme can refuse partial refresh.** `ThemeLayout.supports_partial_refresh=False` makes `OutputService.publish()` build the driver with `enable_partial=False` regardless of config; the flag can only remove partial refresh, never add it. Declared by two families: the dithered plates (`constellation_map`, `day_arc`, `halftone`, `halftone_agenda`, `moonphase`, `moonphase_invert`, `moonphase_photo`, `naturalist`, `photo`, `postcard`, `trends`, `weatherglass`), because the fast waveform fades their artwork in bands aligned with it — the report that drove this was `halftone_agenda`, whose split plate faded the agenda alongside the engraving (#222) — and the dark-canvas themes (`fantasy`, `qotd_invert`, `terminal`), whose `ThemeStyle.bg` is ink so the whole plate is one solid fill. `fuzzyclock_invert` is dark-canvas too (95% ink, the heaviest on any theme) and keeps partials **on purpose**: its phrase changes every 5 minutes, so declining would mean ~200 full-waveform refreshes a day. It is named in `SOLID_INK_BY_CHOICE` in `tests/test_theme_partial_refresh.py`, which pins both families and fails any new `canvas_mode="L"` or `bg=0` theme that claims partial support without being listed there; `validate_config()` warns naming the configured opt-outs when `display.enable_partial_refresh` is on.
