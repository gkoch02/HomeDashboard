...
- Theme preview PNGs live under `assets/previews/` (committed to git as a normal tracked directory — no `.gitignore` exceptions needed). `make previews` and `scripts/build_split_previews.py` both write there. The legacy `output/` directory is now purely a runtime-artifacts dir (dry-run scratch + `latest.png` + `last_success.txt`)
...
- `theme_rules` (evaluated by `services/theme_rules.py`) is first-match-wins. Each rule's `when:` block accepts any subset of `weather` (OWM description substring — scalar or list), `weather_alert_present` (bool), `daypart` (`dawn`/`morning`/`afternoon`/`dusk`/`night`, plus `day` as an alias for morning ∪ afternoon), `season` (`spring`/`summer`/`fall`/`autumn`/`winter`), `weekday` (weekend / weekday / day name), and `calendar` (`empty` / `done` / `active` / `upcoming_soon` / `busy` / `birthday_today`). Unset fields don't constrain. Rules needing weather or calendar data silently skip when data is unavailable. `DashboardApp.run()` calls `resolve_theme_name()` twice when rules exist (once pre-fetch with `data=None`, once post-fetch); if any rule could resolve to `monthly`, the pre-fetch event window is widened for the month grid via `_rules_can_pick_monthly()`. When no rules are configured, the second call is skipped.
