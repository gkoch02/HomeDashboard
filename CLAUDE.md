...
| `NuCore.otf` / `NuCore Condensed.otf` | `nucore`, `nucore_condensed` | `sunrise`, `tides`, `scorecard` (high-contrast display numerals) |
...
- **The version has one home: `src/_version.py`.** `pyproject.toml` declares
  ...
  `## [Unreleased]` changelog block, commits, and tags `vX.Y.Z` in one step (rolling all of it
  back if any step fails, so a rejected commit hook doesn't strand a half-cut
  release); the bump size
...
- eInk partial refreshes degrade quality; full refresh forced after `max_partials_before_full` partials. The partial branch in `WaveshareDisplay.show()` calls `epd.init_fast()`, whose waveform does not drive black as deeply as `epd.init()` — solid fills read closer to charcoal than ink, which surfaces as "the blacks look grey" bug reports. `enable_partial_refresh` defaults to `False` in `DisplayConfig` and `config.example.yaml` now matches (it used to ship `true`, so every template-derived install had it on).
- **Partial-refresh capability is derived, not declared.** `Theme.allows_partial_refresh` (`src/render/theme.py`) resolves it from the plate via `plate_needs_full_waveform(layout, style)`, and `OutputService.publish()` builds the driver with `enable_partial=False` for any theme that resolves `False`, regardless of config; the resolution can only remove partial refresh, never add it. A plate declines when it dithers — `preferred_quantization_mode` of `floyd_steinberg`/`ordered`, or a dithered `background_fn` — or when `ThemeStyle.bg` is ink (int `0` or an all-zero RGB triple), so the whole plate is one solid fill. **`canvas_mode == "L"` is NOT the criterion**: `threshold` is a hard cut that diffuses nothing, which is why the L-mode `moonphase_invert` and `weatherglass` keep partials. `ThemeLayout.supports_partial_refresh` defaults to `None` ("derive") and exists only to **overrule** the derivation — `fuzzyclock_invert`, `moonphase` and `moonphase_photo` set `True` on plates that would otherwise decline, each with its reason at the theme and its name in `OVERRIDES` in `tests/test_theme_partial_refresh.py`. That module also rejects a declaration that merely agrees with the derivation, so the only flags in the tree are real decisions. Known boundary: the derivation reads the theme, never `display.quantization_mode`, so an L-mode theme declaring no quantizer would inherit a global `floyd_steinberg` unseen — every L-mode theme declares one except `constellation_map`, which declines on the ink rule anyway. The plates that decline today are the ones that **dither** — a `preferred_quantization_mode` of `floyd_steinberg`/`ordered` (`day_arc`, `halftone`, `halftone_agenda`, `naturalist`, `postcard`, `trends`) or a dithered `background_fn` (`photo`) — because the fast waveform fades their artwork in bands aligned with it (the report that drove this was `halftone_agenda`, whose split plate faded the agenda alongside the engraving, #222); and the **dark-canvas** ones, `ThemeStyle.bg == 0`, whose whole plate is one solid fill (`constellation_map`, `fantasy`, `qotd_invert`, `terminal`). The three plates listed in `OVERRIDES` keep partials on purpose: `fuzzyclock_invert` (redraws every 5 min, so declining means ~200 full refreshes a day) and `moonphase`/`moonphase_photo` (no fade observed on hardware — no fine detail to band, and the flash is worst on a theme left up at night). `validate_config()` warns naming the configured opt-outs when `display.enable_partial_refresh` is on.
...
- `canvas_mode = "L"` themes must use `bg=255` (not `bg=1`) in `ThemeStyle` for a light-canvas theme, or `fg=255, bg=0` for a dark-canvas theme like `constellation_map`; `bg=1` is near-black in L mode. Every built-in theme except `constellation_map` uses `canvas_mode="1"` (default) and is unaffected
- Image hash comparison (`output/last_image_hash.txt`) skips eInk writes when content unchanged. This makes anything drawn from the render clock expensive: a caption reading `now` differs every tick, so the hash differs, so the panel is written — ~12 times an hour on the default 5-minute timer, to repaint a few hundred pixels. On Waveshare with `enable_partial_refresh` on, those writes go through `epd.init_fast()` and static ink drifts grey between full refreshes. **"Updated" captions must draw `DashboardData.content_at` via `primitives.content_time(data, now)`, never `now`.** `content_at` is the newest timestamp among the sources actually backing the snapshot — a live fetch stamps it with the fetch time, a cached source carries the cache's own timestamp forward — so it only moves when the data does. `DashboardData.fetched_at` is a render clock and advances every run regardless. `tests/test_idle_tick_no_redraw.py` renders every theme at two ticks with nothing fetched and fails any theme whose image changes; themes whose content genuinely tracks the clock (fuzzyclock, day_arc, light_cycle, timeline, and the dateline-clock themes) are listed in its `TIME_DRIVEN` exemption set, which is itself asserted to stay accurate.
...
- `output/latest.png` is **the current render, not a mirror of the panel**. `OutputService.publish()` writes it after a successful hardware write *and* on the cooldown-suppression path, where the content changed but `display.min_refresh_interval_seconds` has not elapsed — otherwise the web UI's "current display" image showed the previous frame for the whole cooldown, an hour for anyone who sets `3600` on Inky to restore the v4 hourly throttle (#245). The unchanged-hash path deliberately skips the write: the file already matches by definition. A deferred run still does **not** persist the image hash, so the next eligible run paints the pending change.
- `DryRunDisplay.show()` writes a timestamped `output/dashboard_<ts>.png` beside `latest.png` and then prunes to the newest `DRY_RUN_HISTORY` (20). Nothing used to remove them: they are gitignored (`output/*.png` with a `!output/latest.png` exception), so they never appeared in `git status` while filling the card at ~50–100 KB each, and `make dry` / preview loops produce them in bulk. Pruning sorts by name, which for `dashboard_YYYYmmdd_HHMMSS.png` is chronological and does not depend on mtimes; a failure to unlink is logged at DEBUG and never fails a preview.
...
- **One Call is the only paid part of the weather fetch, and its version is a config choice.** Current conditions + forecast come from the free `/data/2.5/weather` and `/data/2.5/forecast` endpoints; alerts and `uv_index` are the only two values sourced from One Call, via `src/fetchers/weather_onecall.py`. `weather.one_call_version` selects `"3.0"` (default), `"4.0"`, or `"off"`. OpenWeather sells 3.0 and 4.0 as separate products and **an account can hold only one subscription**, so calling the wrong one returns 401 — indistinguishable from having none, which is why this cannot be auto-detected. The parser (`_normalise_one_call_version` in `config.py`) folds the YAML traps back onto the canonical strings: unquoted `3.0` is a float, `3` is an int, and `off` is the YAML 1.1 boolean `False`. **The silence is scoped, not total.** `src/fetchers/one_call_health.py` classifies the failure at the degradation boundary: a 401/403 is permanent and actionable, everything else (timeouts, 5xx, unexpected payloads) is transient. The permanent case logs at `WARNING` once on the healthy→failing transition and shows on the status page as a degraded One Call row naming `weather.one_call_version`; the transient case stays at `DEBUG` as before. Classification reads `exc.response.status_code` and never the exception text — a "401" in a message is not evidence of a 401 response. Deduplication has to survive across runs (the renderer is a short-lived timer job), so the outcome is persisted to `state/one_call_health.json`; an unwritable state dir degrades to classification-and-logging only, because health bookkeeping must never break a fetch. The status page reconciles that record against the configured version before showing anything, because the record only refreshes on the next weather fetch and the `off` path deliberately records nothing — without it, both documented remedies (switch to `off`, or switch 3.0 ↔ 4.0) leave a banner up naming a setting the user already changed. A render is still never broken by a One Call failure.
...
- ICS recurrence (RRULE / RDATE / EXDATE / RECURRENCE-ID) is expanded via the `recurring-ical-events` package (core dependency) in `_expand_components()` — one VEVENT per occurrence inside the fetch window, matching the CalDAV backend's `server_expand=True`. If the package is missing (stale deployment), the fetcher logs a warning and falls back to the raw walk, where recurring series appear only in their first week. Two properties of the expander are load-bearing and each has a regression test in `tests/test_ical_edge_cases.py`: (1) **it raises on bad input** — `KeyError('DTSTART')` for a VEVENT without `DTSTART`, `BadRuleStringFormat` for an unparseable `RRULE` — and a feed-wide fallback to the raw walk would silently revert *every* series in the calendar to first-week-only, so `_drop_unusable_vevents()` prunes DTSTART-less VEVENTs first (keeping VTIMEZONE, which the expander needs for TZIDs) and `_expand_one_by_one()` retries per component on any remaining failure; (2) **it resolves floating `DTSTART`s (no `TZID`, no `Z`) against UTC** while the caller's window filter resolves them against the configured zone, so the expansion span is padded by `_EXPAND_PAD` (1 day, exceeding the largest real UTC offset) either side — without it a western-zone install drops short floating events in the first `|utcoffset|` hours of day one. The pad never widens the window: the existing per-event filter still decides what is in range. Occurrences are also capped per series at `_MAX_OCCURRENCES_PER_SERIES` (500) by `_cap_runaway_series()` — an unbounded `FREQ=MINUTELY` rule expands to five figures in a week. The cap is per series, not per feed, because `between()` groups occurrences by series rather than sorting them, so a flat head-of-list cap would keep the runaway series and drop the real events behind it
- `_fetch_from_ical()` uses `requests.get(..., timeout=_REQUEST_TIMEOUT_SECONDS)` (30s) and the `icalendar` library; an HTTP or parse error is logged and raises `CalendarFetchError` **immediately**, ending the walk. Collecting every feed's failure first cost one timeout per feed and `retry_fetch` then repeated the sequence, so three dead feeds spent ~180s against the pipeline's 120s per-source ceiling — and since the pipeline cannot kill the worker thread, the renderer process stayed alive until the walk finished. Any failure discards the whole result, so the feeds after the first are pure waste
- **A calendar backend fails rather than reporting a short calendar** (`src/fetchers/errors.py`). `DataPipeline._resolve_source` treats any non-raising return as the complete, authoritative answer — it writes it to the cache, marks the source `FRESH`, and records a breaker **success** — so the ICS and CalDAV backends returning `[]` on an outage overwrote the last good calendar with an empty one, left no staleness glyph, and kept the breaker closed so nothing ever fell back to cache (#234). Both now raise `CalendarFetchError`, and the pipeline's existing cache-fallback path renders the last complete calendar with the stale indicator. This applies to **partial** failure too: with several `additional_ical_urls`, one failed feed fails the fetch rather than returning the others, because there is no way to express "partial" in the value the pipeline caches — the working feeds still reach the panel from cache. `CalendarFetchError` deliberately subclasses `Exception` and none of `RuntimeError`/`ValueError`/`TypeError`/`KeyError`, the four `retry_fetch` treats as permanent, so a transient network failure still gets its one retry. A genuinely empty week is still `[]`, cached normally — only a failed *fetch* is a failure
...
- `theme_rules` (evaluated by `services/theme_rules.py`) is first-match-wins. Each rule's `when:` block accepts any subset of `weather` (OWM description substring — scalar or list), `weather_alert_present` (bool), `daypart` (`dawn`/`day`/`dusk`/`night` — the four buckets `_current_daypart()` returns and `_VALID_DAYPARTS` accepts; `day` is a bucket in its own right, not an alias for finer ones), `season` (`spring`/`summer`/`fall`/`autumn`/`winter`), `weekday` (weekend / weekday / day name), `calendar` (`empty` / `done` / `active` / `upcoming_soon` / `busy` / `birthday_today`), `temp_at_least` / `temp_at_most` (inclusive bounds in the configured `weather.units`), and `aqi_at_least` (inclusive EPA AQI floor). Unset fields don't constrain. Rules needing weather, air-quality, or calendar data silently skip when data is unavailable — for `aqi_at_least` that includes every install without PurpleAir configured, since the source is then never fetched. A numeric threshold `load_config()` cannot read drops the whole rule rather than widening it to "no constraint" (an unreadable bound would otherwise match everything and shadow every rule below it); booleans are rejected because YAML 1.1 reads `yes` as `True` and `int(True)` is a plausible-looking 1. `_optional_number` normalises *every* unreadable shape to `ValueError`, including the YAML list and mapping that `int()`/`float()` reject with `TypeError` — letting that escape would crash `load_config()` itself, taking down every renderer run, `--check-config`, and both web pages over one malformed rule. `config_editor._normalise_patch()` shape-checks the same thing so a web save cannot silently discard the rule it just accepted. `DashboardApp.run()` calls `resolve_theme_name()` twice when rules exist (once pre-fetch with `data=None`, once post-fetch); the pre-fetch event window is sized by `_event_window()`, which fetches the **union** of the ranges of the pre-fetch pick and every theme a `theme_rules` entry could resolve to. It unions rather than picking one because the candidates are anchored differently — `monthly` to the Sunday-first month grid, `THEMES_NEEDING_TOMORROW` to the week anchor plus 8 — so neither is reliably the superset. The earlier version picked by fixed priority with `monthly` winning unconditionally, and `monthly`'s grid ends on the last day of the month: when a month ends on a Saturday it did not reach tomorrow, so a post-fetch flip to `day_arc` / `halftone_agenda` that day left their after-dark rollover with nothing to show (2026: Jan 31, Feb 28, Oct 31). A `None` start means the fetchers' default anchor, resolved via `_time.week_start()` — the same helper the three calendar fetchers use, so the union can't drift from the window they actually fetch — and the result keeps `None` when the union begins on that anchor, so the common single-candidate case is byte-identical and no cached events window is needlessly invalidated. When no rules are configured, the second call is skipped.
- Calendar states (computed in `_calendar_states()` in `services/theme_rules.py`) are non-exclusive: `empty` (no events cover today), `done` (≥1 timed event today, all `end <= now`), `active` (timed event with `start <= now < end`), `upcoming_soon` (timed event starts within `UPCOMING_SOON_MINUTES` = 30 min), `busy` (≥ `BUSY_DAY_THRESHOLD` = 5 events covering today), `birthday_today` (any `Birthday` whose month/day matches today). All-day events use iCal inclusive-start/exclusive-end overlap (same as `events_for_day` in `primitives.py`) and count toward `empty`/`busy` but not `active`/`done`/`upcoming_soon`. Event-derived states are only emitted when `"events" in data.source_staleness` (and `birthday_today` requires `"birthdays"`), so a fetch outage with no cache doesn't falsely fire `empty`. `now` is normalized to naive local time inside `_calendar_states` because fetchers strip tzinfo from `CalendarEvent.start/end` while `DashboardApp._resolve_now()` hands in an aware `now`.
...
- Quote loading lives in **one** place: `src/render/quotes.py`. Every panel that shows a quote (`info`, `qotd`, `almanac`, `postcard`, `naturalist`, `tides`, `scorecard`, `moonphase`) calls it with its own key prefix — `""`, `"tides-"`, `"scorecard-"`, `"moonphase-"` — so two panels on one plate never show the same quote on the same day. The store path comes from `quotes.path` in config (empty = bundled `config/quotes.json`) and is threaded through `render_dashboard(quotes_path=...)` → `RenderContext` → the panels, not parked in module state, so components stay pure. A missing, malformed, or empty store falls back to the bundled `DEFAULT_QUOTES` and `validate_config()` warns; individual entries lacking a string `text`/`author` are dropped (panels index both directly, so one bad entry would otherwise raise out of whichever panel selected it). `make deploy` rsyncs the whole tree and spares only `config/config.yaml`, so a customised store belongs outside the repository (or pass `make deploy QUOTES_FILE=config/quotes.json`).
...
- `moonphase` theme is a full-canvas display using the `moonphase_full` region on `ThemeLayout`; `draw_moonphase()` in `moonphase_panel.py` receives the full `DashboardData` object (same pattern as `diags_panel` and `air_quality_panel`). Moon phase data is purely computational via `moon.py` and `astronomy.py` — no API needed. Both `moonphase` and `moonphase_invert` use `canvas_mode="L"` with `preferred_quantization_mode="floyd_steinberg"` and `prefer_color_on_inky=True` (dark theme `fg=255, bg=0`; invert `fg=0, bg=255`).
- The hero and ±3-day flanking moons are drawn by `src/render/moon_render.py` (`render_moon_disc()` + `MoonTones`), NOT a Weather-Icons font glyph. By default they are **procedural**: a true phase terminator (lit scanlines bounded by `xt = xlim·cos(2π·phase)`; waxing lights the right limb, waning the left), maria + craters on the lit side only, and flat earthshine on the unlit side so the dark limb stays faintly visible. The renderer is mode-aware — supersampled 4× + LANCZOS on `"L"`/`"RGB"` canvases (pasted through a circular mask), flat 1× on `"1"` (the unit-test fallback). The panel pulls the backing PIL image from `ctx.image` (falling back to `draw._image`) and picks `MoonTones` from the canvas mode + `style.bg` luminance (`_moon_tones`): warm yellow lit / blue earthshine on Inky RGB, greyscale on Waveshare L.
- **Photo occlusion** is opt-in via `ThemeStyle.use_moon_photo` (set only by the `moonphase_photo` theme; the default `moonphase`/`moonphase_invert` stay solid-disk). When the flag is set and a real photo is bundled at `assets/moon_full.png` (`_load_moon_photo()`/`moon_photo_available()` — center-square-cropped, lru-cached), `render_moon_disc(..., use_photo=True)` calls `_render_photo_disc()` to occlude the photo by the phase terminator (`_build_lit_mask()`/`_lit_span()`): the sunlit region shows the photo, the unlit region keeps a dimmed (`_EARTHSHINE_FACTOR = 0.16`) copy of the same texture. Inky (`"RGB"`) renders realistic greyscale (no warm tint); the light parchment polarity (`dark_canvas`, inferred from `MoonTones` luminance when omitted) luminance-inverts the photo to a dark engraving. Falls back to the procedural disc when the asset is missing, and always on `"1"` bilevel canvases. **Replacing `assets/moon_full.png` changes the `moonphase_photo` pixel-snapshot hash — regenerate with `UPDATE_SNAPSHOTS=1 pytest tests/test_theme_pixel_snapshots.py`.**
- The `moonphase` lunar-data block (below the filmstrip) shows moonrise/moonset via `astronomy.moon_times()` (only when `weather.latitude`/`longitude` are set — exact `0,0` is treated as unset, same convention as `astronomy`/`light_cycle`; times are UTC and localized via `now.tzinfo`), sunrise/sunset + compact weather, moon age, and a countdown to whichever of `next_full_moon()`/`next_new_moon()` (`moon.py`) comes first. A `* SUPERMOON *` badge replaces the plain illumination subtitle when illumination ≥ 97% and `astronomy.moon_distance_earth_radii()` < 57.4 ER (full moon near perigee). `latitude`/`longitude`/`now`/`image` are plumbed into `draw_moonphase()` from `RenderContext` via `_builtins.py`.
- `moon_illumination(d)` in `moon.py` returns 0.0–100.0 using a cosine approximation from the phase age; used by the `moonphase` theme's illumination display and supermoon check.
- `astronomy.moon_times(d, lat, lon, tz=None)` scans the local day in 10-min steps for the moon's altitude crossing `MOON_RISE_ALTITUDE` (0.125°) and bisects each crossing; returns `MoonTimes(rise, set)` as UTC datetimes, either of which may be `None` (the moon rises ~50 min later daily, so some calendar days lack a rise or set). `moon_equatorial()` was refactored to share `_moon_geocentric()` with `moon_distance_earth_radii()` (distance in Earth radii; mean ~60.27, perigee ~56, apogee ~63.8).
...
- `light_cycle` theme renders a full-canvas 24-hour radial clock in `light_cycle_panel.py`. Outer rim carries hour ticks + 00/06/12/18 numerals; the twilight ring fills with progressively denser radial dashes (civil → nautical → astronomical) and a solid wedge for true night, all computed from `src.astronomy.sun_times` when `weather.latitude/longitude` are set (else falls back to OWM-reported sunrise/sunset and shows a single night band). Today's timed events appear as small radial dashes on the inner ring. The centre disc carries DOW + hero day-of-month numeral (Righteous, via `font_date_number`) + month + weather summary; positions use `draw.textbbox()` rather than approximate `text_height` so the tall numeral never overlaps the month label. Inky palette `(INKY_YELLOW, INKY_BLUE)` — yellow accents + blue needle. Included in the random rotation pool.
- `almanac` theme is an Old-Farmer's-Almanac front page in `almanac_panel.py`. Masthead + dateline render in **Astloch** (OFL blackletter, `astloch_bold`); body uses Playfair Display + Cinzel. The 2×2 editorial body grid (Heavens / From the Sky / Week Ahead / Next in the Garden) reuses every existing data source — weather, `src/astronomy.py`, `src/render/moon.py`, `events_for_day`, birthdays, and the same `_quote_for_today` helper as `info_panel`. No new fetcher. Inky palette `(INKY_RED, INKY_BLACK)` — red rules + ornaments + bullets + author small caps on cream. Birthday rollover follows the convention `birthday_bar.py` uses (Feb 29 → Feb 28 in non-leap years) so leap-day birthdays neither drop nor crash. Included in the random rotation pool.
- `postcard` theme is a dithered postcard in `postcard_panel.py`. Canvas is split at x=480: left two-thirds is a Floyd-Steinberg-dithered procedural scene (sky gradient via `_daypart_palette`, two-layer mountain silhouettes, water with horizontal ripple lines, foreground shore + reeds, plus sun / moon / clouds / rain streaks / lightning / snowflakes / fog bands keyed off `_scene_kind(icon)`); right third is the postcard back (cursive greeting, circular red postmark with month+day, perforated postage stamp with moon-phase glyph, four ruled "address" lines for today's events, daily quote at the bottom). A 3 px white gutter + dashed shadow forms the centre crease. `canvas_mode="L"`, `preferred_quantization_mode="floyd_steinberg"`, `prefer_color_on_inky=True`. Inky palette `(INKY_RED, INKY_BLACK)`. Included in the random rotation pool.
- `naturalist` theme is a Victorian botanical plate in `naturalist_panel.py`. Astloch blackletter masthead (`PLATE [Roman]` left + `[YEAR-roman] · [MONTH]` right) over a triple-rule. Below it: Cinzel small-caps Latin specimen name (`QUERCUS VERNALIS`, `AESTIVALIS`, `AUTUMNALIS`, `HIBERNALIS` keyed by `_season(today)`, plus weather suffixes `· sub pluvia / fulmine / nive / nebula / gelu` via `_weather_modifier(icon, temp)`). Hero specimen is a procedural branch (solid black trunk + white engraving-style highlight strokes, curving roots, mixed filled/outlined almond leaves whose count + treatment vary by season — bare in winter, buds in spring, lush canopy in summer, fallen leaves on the ground in autumn — plus weather overlays for rain/storm/snow/frost/fog). RNG seed comes from `_stable_seed(season, modifier, today)` so the specimen is reproducible across Python processes (default `str.__hash__` is randomized per process and would break snapshot tests). Four leader-line callouts (`FIG. I EVENT`, `FIG. II LUNA`, `FIG. III SOL`, `FIG. IV AER`) pin data to anatomical features on the specimen via `_feature_points()`. Triple-rule footer + daily quote in Playfair + red Cinzel author. `canvas_mode="L"`, `preferred_quantization_mode="floyd_steinberg"`, `prefer_color_on_inky=True`. Inky palette `(INKY_RED, INKY_BLACK)`. Included in the random rotation pool.
- `constellation_map` theme is a dark-canvas star chart in `constellation_map_panel.py`. Uses `canvas_mode="L"` with `fg=255, bg=0` (white stars on black sky) and `prefer_color_on_inky=True` so Inky renders the disc rim, cardinal letters, and constellation names in yellow with blue constellation lines + altitude rings. Equidistant azimuthal projection (zenith centre, horizon rim, N top / **E left** / S bottom / W right — looking-up convention). Bundled star catalogue (`src/render/star_catalog.py`) is a curated J2000 subset (~45 named bright stars) with constellation outline data for Ursa Major, Cassiopeia, Orion, Lyra, Cygnus, Boötes, and Leo. Position math (`_julian_day_full`, `gmst_degrees`, `local_sidereal_time`, `equatorial_to_horizontal`, `moon_equatorial`) all live in `src/astronomy.py`. During daylight the chart auto-projects for tonight's solar midnight (12 h after solar noon) so the disc stays informative. Star and constellation labels render in **Audiowide** (OFL retro-futuristic display sans, `audiowide`). Requires `weather.latitude` / `weather.longitude`; without them the panel renders an explanatory message inside the disc. Inky palette `(INKY_YELLOW, INKY_BLUE)`. Included in the random rotation pool.
- `weatherglass` theme is a Victorian weather-station instrument deck in `weatherglass_panel.py` (full-canvas `weatherglass` region on `ThemeLayout`; the panel receives the full `DashboardData` like `diags_panel`). A **Rye** (`rye`) Western-saloon masthead sits over three hero instruments — a procedural mercury thermometer, a round barometer dial, and a hygrometer arc + UV-index bar — then a secondary row of wind compass, sun arc (`src.astronomy.sun_times`), moon porthole, and an optional AQI badge. The barometer carries a second **trend needle** driven by a tiny rolling pressure history in `state/weatherglass_pressure_history.json` (atomic write, trimmed to samples 1–36 h old); `app.py` passes `state_dir=None` on `--dry-run`/`--dummy` runs so previews never persist history. `canvas_mode="L"` at 2× supersample (1600×960) with `preferred_quantization_mode="threshold"` (not Floyd-Steinberg — every shaded zone is hand-stippled, so the antialiased rims snap to crisp solid black instead of dithering into speckle) and `prefer_color_on_inky=True`. Numerals use **Cinzel Black** (`cinzel_black`, via `font_date_number`) since NuCore's digit/degree metrics are degenerate. Inky palette `(INKY_YELLOW, INKY_RED)` — brass rims yellow, mercury column + alert cartouche red, cold scale + falling-pressure needle blue, comfort band + rising-pressure needle green. No new fetcher. Included in the random rotation pool.
- `day_arc` theme is the calendar-forward sibling of `halftone`, rendered by `day_arc_panel.py` via the full-canvas `day_arc` region. Bands: a 160-px dithered ribbon, a 40-px paper-white axis strip, a 6-px Bayer rule, then a 274-px body (agenda left, supporting rail right, footer). Notable points:
  - **Every element in the axis strip gets an exclusive row band** (the `_AXIS_*` constants: ticks+caret, pips, duration bar, labels). They originally shared rows, which made collisions a matter of luck — an event starting on the hour puts its pip at exactly the x its hour label is centred on, so a plain 9a/12p/3p/6p calendar collided on every one. `TestAxisStripBands` fails the build if the bands stop being pairwise disjoint. The 8 px this needed came out of `SKY_H` (168 → 160) so `RIBBON_H` stays 200 and the agenda geometry below is untouched.
  - **The axis strip is deliberately white.** Ticks, labels and pips drawn onto the sky itself would vanish into the night gradient at one end of the day and wash out against the midday band at the other; a paper strip keeps every mark in solid ink at all hours.
  - **`build_time_axis()` is piecewise-linear**, not linear: the daylight core always gets `DAY_FRACTION` (78%) of the width and the night margins are compressed, so a midwinter day still reads as a day while a 21:00 dinner keeps a truthful position. It requires events **already filtered to the day** (`events_for_day`) and hard-clamps to `AXIS_MIN_HOUR`/`AXIS_MAX_HOUR` — passing a whole week once stretched the axis across six days. Falls back to a single linear segment when sun times are missing, when `sunset <= sunrise`, or when the clamped bounds swallow the sunrise/sunset pair (polar).
  - **Sun times** come from `astronomy.sun_times` when `weather.latitude`/`longitude` are set (exact `(0.0, 0.0)` counts as unset, same convention as `astronomy`/`light_cycle`/`moonphase`), else from OWM-reported `weather.sunrise`/`sunset`, else `None` → fixed 05:00–23:00 window.
  - **Event state is encoded by dithering**: past rows are Bayer-perforated via `skyart.screened_paste()`, the in-progress event is inverted into a solid accent bar, upcoming rows are crisp. Screening happens *after* typesetting rather than by drawing mid-grey — Floyd-Steinberg diffuses error across glyph boundaries and turns small type to mush, and the Inky RGB path never runs FS at all, so a mid-grey row would screen on one backend and not the other. `screened_paste` emits pure 0/255, which FS passes through untouched.
...
- `halftone_agenda` is the split-plate sibling of `halftone`, rendered by `halftone_agenda_panel.py` via the full-canvas `halftone_agenda` region. The left 372 px carry the shared `skyart.draw_weather_scene` illustration (at `SCENE_SCALE = 0.72`) over a typeset weather band; a 6-px vertical Bayer rule divides the plate; the right 422 px are the agenda. Notable points:
  - **Calendar semantics are imported from `day_arc_panel`, not re-derived** — `agenda_day` (the after-dark, all-events-ended rollover behind the `TOMORROW` dateline). A second copy of the rollover rule would be free to drift out of agreement with the first.
  ...
  - **The density tiers are this pane's own** (`_DENSITY_TIERS`, five of them): the column is half as wide and half again as tall as `day_arc`'s, so reusing that table set 32-px rows in a 400-px pane. `agenda_metrics` picks by what fits, same as its `day_arc` counterpart. Row heights are sized to their type rather than stretched to fill the pane, and the list is top-justified under the rule — a stretch-to-fill pitch with the short list centred left two events floating in the middle of the pane, reading as a layout fault rather than a light day.
  - **Both typeset regions are hardened before the backend quantizes** (`skyart.harden_typeset` on the weather band and the agenda pane). Without it Floyd-Steinberg eats the antialiased glyph edges — the effect is small in pixel count (~500 px on the plate) and very visible on the panel. The illustration is deliberately outside both rects so it still dithers.
  - **The weather band's display type is sized to the widest real content**, not chosen by eye: `TEMP_PT = 78` is the largest at which `"108°"` still fits `TEMP_COL_W` and the widest OWM phrase (`heavy intensity rain`) still wraps as `HEAVY INTENSITY / RAIN` in what's left; 82 pt breaks it as `HEAVY / INTENSITY RAIN` and 86 pt needs a third line. A phrase needing more than two lines has its overflow folded into line two so `draw_text_truncated` ellipsizes it — otherwise `thunderstorm with light drizzle` renders as a complete-looking `THUNDERSTORM WITH LIGHT`. Tests pin all three bounds.
  - **Rows stack the end time under the start** (`start –` / `end`) rather than setting an inline range. An inline range's width depends on the times themselves — `11:30a–1:15p` is half again as wide as `12:30–2p` — so at one density some rows would show an end time and their neighbours wouldn't. Stacked, the cell is never wider than one label plus the dash, which every column already fits, and the only remaining question is vertical: `two_line_time_fits` is true for every tier but the densest, whose 33-px rows are 4 px short, and that tier shows the start alone. `event_times` also falls back to the start for all-day events and for one running past midnight.
  - **The pane encodes each event's state in its rendering** — elapsed rows Bayer-perforated via `skyart.screened_paste`, the event in progress inverted into a solid bar filled with `artkit.accent_red` (red on Inky, ink on Waveshare), the next timed event's tick accented, and a rolled-over agenda behind an inverted `TOMORROW` chip. All four were removed once, to buy partial-refresh compatibility the plate never had, and restored when the theme took `supports_partial_refresh=False` instead (#222) — they need the full waveform, and now they always get it. `past_screen(title_pt)` raises the Bayer cut as the type shrinks (96 → 128 → 160): one fixed cut leaves the packed tiers' 17-px type as a row of dots. `+N more` stays plain ink — screened, it is the faintest thing on the plate, and it is the one line saying the day continues. State is the only thing here reading the clock and only at event boundaries, so an idle tick still renders byte-identically (`tests/test_idle_tick_no_redraw.py` keeps the theme out of its `TIME_DRIVEN` set for exactly that reason).
  ...
  - **It is in `THEMES_NEEDING_TOMORROW`** (`src/app.py`) alongside `day_arc`. The default event window is Monday-anchored and 7 days long, so on a Sunday it stops before tomorrow — any theme that rolls its agenda over after dark needs the extra day fetched or it reports "Nothing scheduled" every Sunday evening. Add new rollover themes to that set.
  - **The agenda's DM Sans runs one weight heavier than the role it fills** — `font_bold` for titles, `font_semibold` for times, `font_medium` for locations and the footer — and the theme points `font_bold` at `dm_bold` rather than Righteous (the display face is reached through `font_title`/`font_section_label`/`font_date_number`). Both panes are pure black on pure white by the time the panel sees them, so "not black enough" is stroke mass, not tone: at 22 px Righteous sets ~4.4 px stems and DM Sans SemiBold ~3.7 px, which reads as grey text beside it. DM Sans Bold at 22 px is ~4.4 px, matching the weather pane.
  - Typography is role-split like `day_arc`'s: Righteous for the weather pane and agenda chrome, DM Sans for the event rows. Inky palette `(INKY_YELLOW, INKY_RED)` — yellow rings the sun in the art pane, red marks the running event's bar and the next-up tick in the agenda. Included in the random rotation pool.
...
- `numpy` and `caldav` are declared in core `[project].dependencies` (not just `[dev]`/`[web]`). `numpy` is because `src/display/driver.py` (`InkyDisplay.show`/`clear`) and `src/render/quantize.py` (fast path) import it unconditionally at runtime. `caldav` is because `src/fetchers/calendar_caldav.py` imports it inside `fetch_from_caldav` only when `cfg.google.caldav_url` is set — but declaring it in core lets `pip install .` work end-to-end without an extras pin. The `test-core-install` CI job runs the non-web test suite against a bare `pip install . pytest` (no `[dev]`/`[web]` extras, no `flask`/`waitress`) to guard against core code drifting toward optional-extra packages. Web tests are explicitly excluded via `--ignore-glob='tests/test_web_*.py'` because they legitimately require the `[web]` extras. **That pytest step cannot see a broken install.** Three mechanisms put the repo root on `sys.path` ahead of site-packages — `tests/__init__.py` makes pytest prepend the rootdir, `python -m pytest` prepends the cwd, and `pythonpath = ["."]` adds it a third time — so `import src.*` there always resolves to the checkout, which is how the job stayed green through an install that scattered every module across the top level of site-packages. The job therefore has a separate step that imports the installed package with `working-directory: /tmp`, where the source tree is not importable; it runs first so it fails fast. Add new top-level subpackages to that import list. The install itself is pinned by `[tool.setuptools.packages.find] include = ["src*"]` — without it, auto-discovery reads the repo as a src-layout and installs `app.py`, `config.py`, `render/`, `web/` and friends as top-level names, which is both unimportable (every module does `from src.… import`) and a collision risk against unrelated packages.
- **A state file with two writing processes must be changed through `locked_update_json()`, not a bare `atomic_write_json()`.** `atomic_write_json` makes a single write atomic — it cannot be torn — but it says nothing about interleaving, and `state/dashboard_cache.json` and `state/dashboard_breaker_state.json` are written by both the long-running web service and the 5-minute renderer. Read-all → mutate → write is a losing sequence across processes: whichever writes last erases the other's change, so `POST /api/reset-breaker` could report success while the breaker stayed open, and a source the renderer had just refreshed could be resurrected from the web process's stale read (#242). `src/_io.file_lock()` takes an advisory `fcntl.flock` on a **sidecar** (`<file>.lock`) — the sidecar and not the file itself, because an atomic write replaces the target's inode and a lock held on the old one guards nothing — and `locked_update_json(path, mutate, default=…)` wraps the read *and* the write in it. Callers: `web/routes/actions.py`, `fetchers/cache.save_source()`, `CircuitBreaker._save()`, and `web/event_store.append_event()`. `CircuitBreaker` additionally tracks a `_dirty` set and merges **only the sources this run changed**: writing its whole in-memory `_states` back would undo a web reset made since it loaded, even under the lock. Missing `fcntl` degrades to a no-op — bookkeeping must never be what fails a render.
- `atomic_write_json()` lives in `src/_io.py` and is the only sanctioned way to persist JSON state from the renderer. Callers: `fetchers/cache.py` (the per-source cache), `fetchers/circuit_breaker.py`, `fetchers/quota_tracker.py`, `fetchers/calendar_google.py` (sync state), `fetchers/one_call_health.py`, `display/refresh_tracker.py`, and `web/routes/actions.py`. There is one implementation — the web layer imports the same helper rather than keeping the inline copy it used to carry. Any new persistent state should go through it.
