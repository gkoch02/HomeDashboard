...
A full-height vertical Bayer rule divides the panes. The **right pane** (422 px) is the calendar at the size a dedicated column allows: a TODAY header with the event count, then as many rows as fit at a legible size — five density tiers from two roomy rows with locations down to eleven dense ones, with `+N more` beyond that. Every row carries its event's start and end time: on one line when both fall on the hour (`10a–12p`), otherwise with the end stacked under the start (`12:30p –` / `2p`) so the cell is never wider than a single label. Only the densest tier, whose rows are too short for a second line, shows a stacked pair's start alone — as do all-day events and any event running past midnight. Like [`day_arc`](#day_arc), the pane encodes each event's state in its rendering: elapsed rows are perforated on a Bayer lattice, the event in progress inverts into a solid bar, the next one up carries an accented tick (red on Inky, ink on Waveshare), and a rolled-over agenda sits behind an inverted `TOMORROW` chip. Those treatments were removed at one point and put back: each needs a large or dithered area of ink to survive the panel, which a partial refresh cannot deliver — but the engraving on the left pane is dithered ink too and faded in bands running straight across the agenda's rows, so the plate was never going to survive a partial refresh whatever the calendar side did. The theme now declines partial refresh outright and always takes the full waveform (see [Themes that always refresh fully](configuration.md#themes-that-always-refresh-fully)), which is the condition the treatments needed. State is the only thing here reading the clock, and only at event boundaries, so a tick that crosses none renders identically and costs no hardware write. After sunset, once every timed event has ended, the pane rolls over to tomorrow — the same two-part condition `day_arc` uses. An "updated" caption closes the bottom corner.
